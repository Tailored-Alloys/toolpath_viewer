...
/// Render the wait time gradient scale panel.
///
/// Identical layout to the parameter gradient scale but uses wait time units
/// and its own filter min/max.
pub fn show_wait_gradient_scale(
    ctx: &Context,
    region: &GradientRegion,
    ...
    egui::Area::new(egui::Id::new("wait_gradient_scale_area"))
        .fixed_pos(region.pos)
        .order(egui::Order::Foreground)
        .interactable(true)
        .movable(false)
        .show(ctx, |ui| {
            egui::Frame::none()
                ...
                .show(ui, |ui| {
                    ...
                    let panel_inner_w = region.width - 24.0;
                    let (content_rect, _) = ui.allocate_exact_size(
                        egui::vec2(panel_inner_w, content_h),
                        egui::Sense::hover(),
                    );
                    ...
                    // Mode label
                    let mode_label = format!("Wait ({})", global_units.time.label());
                    ui.painter().text(
                        egui::pos2(center_x, y + label_h / 2.0),
                        egui::Align2::CENTER_CENTER,
                        &mode_label,
    ...
    GradientOutput {
        filter_min: *wait_filter_min,
        filter_max: *wait_filter_max,
    }
}
